...
void RTCS_Run_Scheduler_EventTrigger(void){
	int i;
	int returnval;
	...
	while (1) {
		...
		g_I2C_Msg.Client = Task_Motion_Sensor;
		returnval = RTCS_Release_Task(g_I2C_Msg.Client);
		}
		else if (g_I2C_Msg.Command == WRITE || g_I2C_Msg.Command == READ)
		{
...
void RTCS_Run_Scheduler_FSM(void){
	int i;
	int returnval;
	SET_BIT(DEBUG_RTCS_SCHED);
